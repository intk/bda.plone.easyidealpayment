...
CREATE_PAY_INIT_URL = "https://secure.easyideal.com/ncol/test/orderstandard.asp"
PSPID = "pspid"
SHA_IN_PASSWORD = "pwd"
...
def create_pay_init(pspid, ordernumber, currency, amount, language, accepturl, declineurl, exceptionurl, cancelurl):
    ...
    signer = easyidealSignature(params, 'sha512', SHA_IN_PASSWORD)
...
class easyidealPay(BrowserView):
    def __call__(self):
        ...
        try:
            ...
            accepturl = "http://webshop-plone.intk.com/easyideal_payment_success"

            declineurl = '%s/@@easyideal_payment_failed?uid=%s' % (base_url, order_uid)
            exceptionurl = '%s/@@easyideal_payment_failed?uid=%s' % (base_url, order_uid)
            cancelurl = '%s/@@easyideal_payment_aborted?uid=%s' % (base_url, order_uid)
    ...
    def getLanguage(self):
        """
        easyideal requires en_EN or en_US language id
        We are parsing the request to get the right
        Note: took this code from getpaid.easyideal (thanks)
        """
...
class easyidealPaySuccess(BrowserView):
    def verify(self):
        ...
        #
        signer = easyidealSignature(data, 'sha512', SHA_OUT_PASSWORD)
